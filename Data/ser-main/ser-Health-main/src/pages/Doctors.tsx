  ...
  const list = useMemo(() => {
    ...
    if (filter) {
      ...
      data = data.filter((d) => {
        ...
        const specAr = (d.specialty_ar || "").toLowerCase(); // Check original Arabic if available
        const specEn = (d.specialty || "").toLowerCase(); // Check original English if available
        
        return spec.includes(q) || q.includes(spec) || 
               specAr.includes(q) || q.includes(specAr) ||
               specEn.includes(q) || q.includes(specEn);
      });
    }
