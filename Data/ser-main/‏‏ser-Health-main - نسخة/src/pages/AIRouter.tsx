...
const AIRouter = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(data);
      toast.success(lang === "ar" ? "✅ تم التشخيص بنجاح" : "✅ Analysis completed via Gemini AI");
    } catch (err: any) {
    ...
    <div className="container px-4 py-6 space-y-6 max-w-lg mx-auto">
      ...
      {result && (
        <div className="space-y-4">
          ...
          {/* Smart Redirection Buttons */}
          <div className="pt-2 space-y-3">
            {result.severity === "high" ? (
              <Link to={`/hospitals?status=available&source=ai`}>
              ...
              <Link to={`/doctors?specialty=${encodeURIComponent(result.specialty)}`}>
                <Button className="w-full h-12 text-lg shadow-lg hover:shadow-xl transition-all" size="lg">
                  ...
                  {lang === "ar" ? `ابحث عن متخصص (${result.specialty})` : `Find ${result.specialty} Specialist`}
                </Button>
              </Link>
            )}

            {result.suggestedTests && result.suggestedTests.length > 0 && (
              <Link to={`/labs?test=${encodeURIComponent(result.suggestedTests[0])}`} className="block">
                <Button variant="outline" className="w-full h-12" size="lg">
                  <FlaskConical className="mr-2 h-4 w-4" />
                  {lang === "ar"
                    ? `البحث عن معمل لـ (${result.suggestedTests[0]})`
                    : `Find Lab for (${result.suggestedTests[0]})`}
                </Button>
              </Link>
            )}
          </div>
